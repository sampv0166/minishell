...
void find_cmd_name(t_pars_tokens *pa_tkns, t_parser_info *pa_info,  int i)
{
    ...
    {
        if(pa_tkns[i].cmd_splitted[pa_info->j] && pa_tkns[i].cmd_splitted[pa_info->j][0] == '<' || pa_tkns[i].cmd_splitted[pa_info->j][0] == '>')
        {
            pa_info->j++;
        ...
        {        
            pa_info->arr[pa_info->len++] =ft_strdup(pa_tkns[i].cmd_splitted[pa_info->j]);
            pa_info->j++;
        }
        else
            break;
...
void find_file_names(t_pars_tokens *pa_tkns, t_parser_info *pa_info,  int i)
{
    while(pa_tkns[i].cmd_splitted[pa_info->j])
    {
        if(pa_tkns[i].cmd_splitted[pa_info->j] && pa_tkns[i].cmd_splitted[pa_info->j][0] == '<' || pa_tkns[i].cmd_splitted[pa_info->j][0] == '>')
