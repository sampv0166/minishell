...
int get_len_new_cmd(char **cmd, int j)
{
    int len;
    int i;
...
int split_cmd(t_pars_tokens *pa_tokens,int i, char **abs_path)
{
    int k;
    int j;
    char **new_cmd;
    ...
    j = 0;
    new_cmd = NULL;
    while (pa_tokens[i].cmd_splitted[j])
    {
        *abs_path = get_abs_cmd(pa_tokens[i].cmd_splitted[j]);
        if(*abs_path != NULL)
        {
            if(access(*abs_path, X_OK) == 0)
            {
                new_cmd = malloc (sizeof (char *) * (get_len_new_cmd(pa_tokens[i].cmd_splitted,j) + 1));
                len = 0;
                while(pa_tokens[i].cmd_splitted[j])
                {
                    if(pa_tokens[i].cmd_splitted[j][0] == '<' || pa_tokens[i].cmd_splitted[j][0] == '>' || \
                    pa_tokens[i].cmd_splitted[j][0] == '|' || ((ft_strlen(pa_tokens[i].cmd_splitted[j]) == 2) \
                    && (pa_tokens[i].cmd_splitted[j][0] == '>')) || ((ft_strlen(pa_tokens[i].cmd_splitted[j]) == 2) \
                    && (pa_tokens[i].cmd_splitted[j][0] == '<')))
                    {
                        j++;
                        continue;
                    }
                    new_cmd[len] = ft_strdup(pa_tokens[i].cmd_splitted[j]);
                    j++;
                    len++;
                }   
                new_cmd[len]  = NULL;
...
int execute_cmd(t_pars_tokens *pa_tokens, int i)
{
    ...
    if(abs_cmd_path == NULL)
    {
        ...
        {
            if(!split_cmd(pa_tokens,i, &abs_cmd_path))
                return (0);
        }
        else
        {
